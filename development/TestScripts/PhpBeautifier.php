...
class PhpBeautifier
{
    ...
    public function process($str)
    {
        ...
        for ($i = 0; $i < $count = count($tokens); $i++) {
            ...
            if (is_array($token) && !empty($token[1])) {
                ...
                } elseif ($token[0] === T_DOC_COMMENT) { //save phpDoc comments
                    $this->docComments[] = $this->processComment($token[1]);
                } elseif ($token[0] === T_CONSTANT_ENCAPSED_STRING) { //save strings
                    $this->strings[] = $token[1];
                } elseif ($token[0] === T_INLINE_HTML) { //save inline html
    ...
    private function indent($str)
    {
        ...
        for ($i = 0; $i < count($strarray); $i++) {
            ...
            } elseif (preg_match("/^or\s/i", $strarray[$i])) {
                $level = str_repeat($this->indent, ($count + 1));
            } else {
    ...
    */
    public function optimizeEcho($matches)
    {
    ...
    * Optimize array keys
    */
    public function optimizeArray($matches)
    {
        if (empty($matches[1])) {
            return $matches[1];
        } elseif (ctype_digit(trim($matches[1])) == true || strpos($matches[1], '$') !== false) {
            return $matches[1];
        } elseif (strpos($matches[1], 'string_replacement') === false) {
            if ($this->tokenSpace == true) {
     ...
     */
    public function folder($source, $target = null)
    {
     ...
     *
     */
    public function file($source, $destination = null)
    {
