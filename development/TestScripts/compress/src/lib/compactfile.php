        ...
        for ($i = 0; $len > $i; $i++) {
            ...
            if (is_string($token)) {
                ...
                if (!$removenext) {
                    ...
                    } elseif (!in_array($nexttoken[0], Compactor::$beforetoken)) {
                        $this->write(' ');
                    }
                }
            } elseif (in_array($token[0], Compactor::$aftertoken)) {
                $removenext = true;
                $this->write($token[1]);
            } elseif (in_array($token[0], Compactor::$removable)) {
                $removenext = true;
            } elseif (in_array($token[0], Compactor::$requires)) { // Remove require + everything until ';', maybe not safe?
                for ($i2 = $i; $len > $i2; $i2++) {
                    $rtoken = & $tokens[$i2];
                    if ($rtoken == ';') {
                        $rtoken = '';
