...
class InvokableTests extends PHPUnit_Framework_TestCase
{
    ...
    {
        $this->smarty->registerFilter('post', new InvokableTests_FilterPost());
        $this->assertEquals("foo[closure-filter-post]", $this->smarty->fetch('eval:foo'));
    }

    public function testFilterOutput()
    {
        $this->smarty->registerFilter('output', new InvokableTests_FilterOutput());
...
{
    public function __invoke($output, $template)
    {
