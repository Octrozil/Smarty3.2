...
<head>
    ...
    <style type="text/css">
        ...
        *}
        ...
        . not-appliccable {} . error {
            background: #FF9999;
        }

        . integer {} . callable {} . float {} . array {} . object {} . resource {} . string {} . directory {} . filename {} . classname {} . line {} . empty,
        .null,
        .boolean,
        .flag {
            font-style: italic;
    ...
    <section>
        ...
        {foreach $data.php.modules as $module}
            <section>
                ...
                <dl>
                    <dt>Available</dt>
                    <dd>{prettyprint value=$module.available}</dd>
                    <dt>Enabled</dt>
                    <dd>{prettyprint value=$module.enabled}</dd>
                    <dt>Version</dt>
                    <dd>{prettyprint value=$module.version}</dd>
                </dl>
    ...
    <section>
        ...
        <table>
            ...
            {foreach $data.properties as $prop}
                <tr>
                    <th id="properties-{$prop@key|escape}"><a href="{$prop.link|escape}"
                                                              title="{$prop.name|escape}">{$prop@key|escape}</a></th>
                    {if $_template}
    ...
    <section>
        ...
        {else}
            {if $data.security.class != 'Smarty_Security'}
                <p>The built-in Security class was extended. It may very well be possible that the following table has
                    no meaning what so ever.</p>
            {/if}
            <table>
                <thead>
                <tr>
