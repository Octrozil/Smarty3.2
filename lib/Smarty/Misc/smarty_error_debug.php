...
class Smarty_Error_Debug extends Smarty
{
    const ALL = 511;
    const PHP = 1;
    const PROPERTIES = 2;
    ...
    */
    ...
    public function __construct(Smarty $smarty, Smarty $tpl_obj = null)
    {
        $this->_smarty = $smarty;
        $this->_tpl_obj = $tpl_obj;
    }

    public function processException(Smarty_Exception $e, Smarty $tpl_obj = null)
    {
        $this->_e = $e;
        if (isset($tpl_obj)) {
            $this->_tpl_obj = $tpl_obj;
        ...
        $j = 0;
        $info = array();
        while (isset($trance[$i])) {
            $t = $trance[$i];
            switch ($t['function']) {
                case '__set':
                case '__get':
                    if ($t['function'] == $trance[$i]['function']) {
                        continue;
                    }
                    $info[$j]['args'] = $t['args'][0];
                    $info[$j]['function'] = $t['function'];
                    $info[$j]['class'] = $t['class'];
                    $info[$j]['line'] = $t['line'];
            }
            $i++;
        }
