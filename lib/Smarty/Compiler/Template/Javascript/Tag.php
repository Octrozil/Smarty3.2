 ...
 * @package Compiler
 */
class Smarty_Compiler_Template_Javascript_Tag extends Smarty_Compiler_Code
{
        ...
        foreach ($attributes as $key => $mixed) {
            ...
            if (!is_array($mixed)) {
                ...
                if (in_array(trim($mixed, '\'"'), $this->option_flags)) {
                    ...
                    // too many shorthands
                    $compiler->error('too many shorthand attributes', $compiler->lex->taglineno);
                }
                // named attribute
            } else {
                $kv = each($mixed);
                ...
                if (in_array($kv['key'], $this->option_flags)) {
                    if (is_bool($kv['value'])) {
                        ...
                        $compiler->error("illegal value of option flag \"{$kv['key']}\"", $compiler->lex->taglineno);
                    }
    ...
    public function closeTag($compiler, $expectedTag)
    {
        if (count($compiler->_tag_stack) > 0) {
            ...
            $compiler->error("unclosed {" . $_openTag . "} tag");

            return;
        }
        // wrong nesting of tags
        $compiler->error("unexpected closing tag", $compiler->lex->taglineno);
            ...
            if ($result[1] == 0) {
                ...
                if ($block) {
                    if ($parameters[0]->getName() != 'content') {
                        $compiler->error("block plugin does not define parameter '{content}'", $compiler->lex->taglineno);
                    }
                    if ($parameters[1]->getName() != 'repeat') {
                        $compiler->error("block plugin does not define parameter '{repeat}'", $compiler->lex->taglineno);
                    }
                ...
                foreach ($parameters as $par) {
                    ...
                    if (isset($params[$name])) {
                        ...
                        $compiler->error("missing required plugin parameter '{$name}'", $compiler->lex->taglineno);
                    }
                }
    ...
    public function buildReflection($callback)
    {
        if (is_string($callback) || $callback instanceof Closure) {
            ...
            throw new Smarty_Exception("callback must be closure, function name (string) , object/class method array or invokable object");
        }
